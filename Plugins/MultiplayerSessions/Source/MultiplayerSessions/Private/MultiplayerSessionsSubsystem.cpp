...
void UMultiplayerSessionsSubsystem::CreateSession(int32 NumPublicConnections, FString MatchType)
{
	...
	if (!SessionInterface->CreateSession(*LocalPlayer->GetPreferredUniqueNetId(), NAME_GameSession, *LastSessionSettings))
	{
		...
		// Broadcast our own custom delegate to see that it failed
		Multiplayer_OnCreateSessionComplete.Broadcast(false);
	}
}
...
{
	if (!SessionInterface.IsValid()) return;
	FindSessionsCompleteDelegate_Handle = SessionInterface->AddOnFindSessionsCompleteDelegate_Handle(FindSessionsCompleteDelegate); // Not our custom delegate
	...
	if (!SessionInterface->FindSessions(*LocalPlayer->GetPreferredUniqueNetId(), LastSessionSearch.ToSharedRef()))
	{
		...
		Multiplayer_OnFindSessionsComplete.Broadcast(TArray<FOnlineSessionSearchResult>(), false);
	}
...
void UMultiplayerSessionsSubsystem::JoinSession(const FOnlineSessionSearchResult& SessionResult)
{
	...
	{
		Multiplayer_OnJoinSessionComplete.Broadcast(EOnJoinSessionCompleteResult::UnknownError);
		return;
	}
...
void UMultiplayerSessionsSubsystem::DestroySession()
{
	if (!SessionInterface.IsValid())
	{
		Multiplayer_OnDestroySessionComplete.Broadcast(false);
		return;
	}
...
void UMultiplayerSessionsSubsystem::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	Multiplayer_OnCreateSessionComplete.Broadcast(bWasSuccessful);
}
...
void UMultiplayerSessionsSubsystem::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	Multiplayer_OnFindSessionsComplete.Broadcast(LastSessionSearch->SearchResults, bWasSuccessful);
}
...
void UMultiplayerSessionsSubsystem::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	...
	Multiplayer_OnJoinSessionComplete.Broadcast(Result);
}
...
void UMultiplayerSessionsSubsystem::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	Multiplayer_OnDestroySessionComplete.Broadcast(bWasSuccessful);
}
